...
async function testSpApiConnection() {
    try {
        ...
        console.log('📞 Tentando buscar pedidos dos últimos 30 dias...');
        ...
        console.log('🔍 Debug - Resposta completa:', JSON.stringify(ordersResponse, null, 2));
        
        if (ordersResponse && ordersResponse.payload) {
            console.log('✅ Conexão com SP-API funcionando!');
            console.log(`📦 Encontrados ${ordersResponse.payload.Orders ? ordersResponse.payload.Orders.length : 0} pedidos dos últimos 30 dias`);
            ...
            if (ordersResponse.payload.Orders && ordersResponse.payload.Orders.length > 0) {
                ...
                console.log('⚠️ API funcionando mas não encontrou novos pedidos no período');
                return true; // API está funcionando
            }
        } else {
            console.log('⚠️ Resposta da API vazia ou sem payload');
            return false;
        }
